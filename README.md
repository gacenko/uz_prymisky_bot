...
безкоштовний і надійний, без потреби в окремому always-on сервері).
...
2. Згадує номер поїзда з `TRAIN_SCHEDULE` у тому ж файлі (6903, 6905, 6909,
   6911, 6913 — тільки ранкові поїзди цього напрямку, що йдуть через
   Княжичі й Березняки в межах активного вікна 7:00-12:00; денні/вечірні
   поїзди тим самим маршрутом (6917, 6923, 6925, 6927) не стежимо).
...
**Один пост може містити кілька потягів одразу** (типовий формат УЗ під час
тривог: "❕З безпекових міркувань затримується" + список з кількох рядків
"Поїзд №... курсує..."). `build_delay_blocks()` у
[scripts/check_channel.py](scripts/check_channel.py) розбирає кожне речення
окремо — інакше при пошуку "першого збігу" в пості легко переплутати дані
чужого потяга з нашим, якщо він згаданий першим (був реальний баг саме на
цьому).

**Відсічка 09:30** (`CUTOFF_TIME`): якщо перерахований час відправлення з
Княжичі все одно виходить раніше 9:30 — сповіщення не шлеться, бо
запізнення не критичне для ранкової поїздки. Якщо затримка настільки велика,
що перерахований час переходить за 9:30 — сповіщення йде з новим часом.
...
Worker (`worker/worker.js`) перевіряє поточний час і день тижня за Києвом
(`Europe/Kyiv`, автоматично враховує перехід на зимовий час) і взагалі не
викликає `workflow_dispatch`, якщо зараз не будній день **7:00-12:00**. Це
не фільтр на рівні даних (як `CUTOFF_TIME` у Python-скрипті), а економія —
GitHub Actions просто не запускається поза цим вікном, бо о іншій порі
(чи у вихідні) перевіряти канал
